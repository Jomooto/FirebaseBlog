class Post {
    ...
    task.on(
        ...
        () => {
          task.snapshot.ref.getDownloadURL()
              ...
              .catch(err =>{
                Materialize.toast(`Error al obtener el url => ${err}`, 4000)
              })
        }
  ...
  subirImagenPostx(file, uid){

    const refStorage = firebase.storage().ref(`imgsComment/${uid}/${file.name}`)
    const task = refStorage.put(file)
    task.on(
        `state_changed`,
        snapshot => {
        ...
        () => {
          task.snapshot.ref.getDownloadURL()
              ...
              .catch(err =>{
                Materialize.toast(`Error al obtener el url => ${err}`, 40000)
              })
        }
